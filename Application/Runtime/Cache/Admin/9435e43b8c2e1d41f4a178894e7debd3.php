...
function addAttrKey() {
	$.post('<?php echo U("addKey");?>',$('#addAttrKey_form').serialize(),function (rs) {
		if(rs){
		    rs = JSON.parse(rs);
		    if(rs['errno'] == 1){
                dataTableObj.fnDraw();
...
function addValue(){
	$.post('<?php echo U("addValue");?>',$('#addAttrValue_form').serialize(),function (rs) {
		if(rs){
		    rs = JSON.parse(rs);
		    if(rs['errno'] == 1){
                layer.closeAll();
...
$.post('<?php echo U("Category/getAllCategory");?>',function (rs) {
	if(rs){
	    rs = JSON.parse(rs);
